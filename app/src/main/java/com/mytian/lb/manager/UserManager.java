...
import com.mytian.lb.bean.user.UpdateActionParam;
...
public class UserManager {
     ...
     */
    public void updateAgreement(Context context, String uid, final Handler handler, final int handlerMsgCode) {
        UpdateActionParam param = new UpdateActionParam();
        param.setUid(uid);
        param.setToken(App.getInstance().getUserResult().getParent().getToken());
        param.setClient_type("1");//0 MB端, 1 LB端
        // 接口参数
        param.setMethod(OpenApiMethodEnum.LOAD_LBAPPOINTLIST);
        param.setParseTokenType(new TypeReference<UpdateActionResult>() {
        });
        // 请求对象
     ...
     * @param handlerMsgCode 返回结果的Handler的Msg代码
     */
    public void updateHabit(Context context, String uid, final Handler handler, final int handlerMsgCode) {
        UpdateActionParam param = new UpdateActionParam();
        param.setUid(uid);
        param.setToken(App.getInstance().getUserResult().getParent().getToken());
        param.setClient_type("1");//0 MB端, 1 LB端
        // 接口参数
        param.setMethod(OpenApiMethodEnum.LOAD_LBHABITLIST);
        param.setParseTokenType(new TypeReference<UpdateActionResult>() {
        });
        // 请求对象
        CommonRequest request = new CommonRequest(param, handler, handlerMsgCode);
